...
const ABSOLUTE_RULES = [
  ...
  {
    title: '⛔ 출력 전 자기검수 체크리스트 (9개)',
    body: [
      '[체크1] body의 모든 치아·부위·영역이 입력 소스에 있는가?',
      '[체크2] body의 모든 치료·장치·술식명이 입력에 있는가?',
      '[체크3] 입력의 "절대/반드시/금지/~만/필수" 강조가 약화·생략되지 않았는가?',
      '[체크4] 입력에 (심함)/(경미) 태그 없는 항목에 "경미한/심한/약간" 수식이 붙지 않았는가?',
      '[체크5] 입력에 없는 치아 문제가 추가되지 않았는가?',
      '[체크6] 입력에 없는 치료 옵션이 추가되지 않았는가?',
      '[체크7] 추측·추론·해석 표현이 남아있지 않은가?',
      '[체크8] 비어있는 섹션 h2가 지워졌는가?',
      '[체크9] 출력에 "#숫자"가 남아있지 않은가?',
      '→ 하나라도 실패 시 body 재작성. 통과 후에만 JSON 출력.',
    ],
  },
]
